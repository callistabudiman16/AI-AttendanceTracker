"""
Attendance Tracker DSL Executor

This module provides a Domain Specific Language (DSL) executor for the Attendance Tracker application.
It can parse and execute DSL scripts written in the Attendance Tracker Language (.dsl files).
"""
...
# Note: We need to import these carefully to avoid circular imports
try:
    from app import (
        load_roster, save_roster, format_date_for_roster,
        find_matching_date_column, process_checkin, process_zoom,
        generate_qr_code
    )
except ImportError:
    # If running standalone, we'll need to import differently
    pass


class DSLExecutor:
    """Executor for the Attendance Tracker DSL"""
    
    def __init__(self, app_instance=None, session=None):
        """
        Initialize the DSL executor.
        
        Args:
            app_instance: Flask app instance (optional, for routes)
            session: Flask session object (optional, for state management)
        """
        ...
        self.commands = {
            ...
            'WAIT': self._cmd_wait,
            'RUN': self._cmd_run,
            'BEGIN BATCH': self._cmd_begin_batch,
            'END BATCH': self._cmd_end_batch,
    ...
    def parse_line(self, line: str) -> Tuple[Optional[str], List[str]]:
        ...
        # Find the command (case-insensitive matching)
        command = None
        args_start = 0
        
        # Try to match commands (longest first)
        for cmd_name in sorted(self.commands.keys(), key=len, reverse=True):
            cmd_upper = ' '.join(tokens[:len(cmd_name.split())]).upper()
            if cmd_upper == cmd_name:
                command = cmd_name
                args_start = len(cmd_name.split())
                break
        ...
        if not command:
            ...
            for cmd_name in self.commands.keys():
                if cmd_name.upper().startswith(first_word):
                    # Check if it's a full match or needs more words
    ...
    def execute_script(self, script_path: str) -> Dict[str, Any]:
        ...
        Args:
            script_path: Path to the .dsl script file
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if self.batch_mode and command != 'END BATCH':
                    self.batch_commands.append((command, args))
                    continue
    ...
    def _cmd_load_roster(self, args: List[str]) -> str:
        ...
        try:
            ...
            self.context['roster_file'] = file_path
            
            return f"Roster loaded: {len(roster_df)} students from {file_path}"
        except Exception as e:
            raise ValueError(f"Error loading roster: {str(e)}")
    ...
    def _cmd_save_roster(self, args: List[str]) -> str:
        ...
        file_path = args[0].strip('"\'') if args else self.context['roster_file']
        ...
        try:
            ...
            raise ValueError(f"Error saving roster: {str(e)}")
    
    def _cmd_download_roster(self, args: List[str]) -> str:
        """DOWNLOAD ROSTER command"""
        return self._cmd_save_roster(args)  # Same as save
    ...
    def _cmd_process_checkin(self, args: List[str]) -> str:
        ...
        # Load check-in file
        if file_path.lower().endswith('.csv'):
            checkin_df = pd.read_csv(file_path, encoding='utf-8')
        else:
            checkin_df = pd.read_excel(file_path, engine='openpyxl')
        
        # This is a simplified version - in reality, we'd call the full process_checkin function
        # For now, return a placeholder message
        return f"Check-in processing initiated for {file_path} (date: {date or 'auto-detect'})"
    ...
    def _cmd_process_zoom(self, args: List[str]) -> str:
        ...
        if not file_path or not os.path.exists(file_path):
            raise FileNotFoundError(f"Zoom file not found: {file_path}")
    ...
    def _cmd_show_late_students(self, args: List[str]) -> str:
        ...
        date_col = None
        try:
            from app import find_matching_date_column
            try:
                # Parse date string
                if '-' in date_str:
                ...
                date_col = find_matching_date_column(df, meeting_date)
            except:
                pass
        except ImportError:
            pass
        
        # If no matching column found, try direct column name match first (exact match)
        if not date_col:
    ...
    def _cmd_show_early_students(self, args: List[str]) -> str:
        ...
        # Filter students with exactly 0.6 points
        early_students = []
        for idx, row in df.iterrows():
            points_value = row[date_col]
            if pd.notna(points_value):
    ...
    def _cmd_show_student_total(self, args: List[str]) -> str:
        ...
        try:
            ...
            idx, confidence, matched = find_student_in_roster(student_name, df, use_gemini=False)
            if idx is not None:
                student_idx = idx
    ...
    def _cmd_run(self, args: List[str]) -> str:
        """RUN command"""
        if not args:
            raise ValueError("RUN requires a script path")
        ...
        if result['success']:
            ...
            raise ValueError(f"Script {script_path} failed: {result.get('error', 'Unknown error')}")
    
    def _cmd_begin_batch(self, args: List[str]) -> str:
        """BEGIN BATCH command"""
        self.batch_mode = True
        self.batch_commands = []
        return "Batch mode started"
    
    def _cmd_end_batch(self, args: List[str]) -> str:
        """END BATCH command"""
        if not self.batch_mode:
            raise ValueError("END BATCH called without BEGIN BATCH")
        
        self.batch_mode = False
        count = len(self.batch_commands)
        
        # Execute all batched commands
        for command, cmd_args in self.batch_commands:
            self.execute_command(command, cmd_args)
        
        self.batch_commands = []
        return f"Batch mode ended: {count} commands executed"


# CLI interface for running DSL scripts
if __name__ == '__main__':
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python dsl_executor.py <script.dsl>")
        sys.exit(1)
    
    script_path = sys.argv[1]
    executor = DSLExecutor()
    
    print(f"Executing DSL script: {script_path}\n")
